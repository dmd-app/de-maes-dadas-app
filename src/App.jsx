...
const ContentSection = ({ title, items, badgeColor }) => (
  <section className="py-6 bg-soft-bg">
    ...
    <div className="flex overflow-x-auto px-6 gap-4 pb-8 snap-x hide-scrollbar">
      {items.map((item, idx) => (
        <div key={idx} className="min-w-[calc(100vw-4rem)] max-w-[calc(100vw-4rem)] md:min-w-[calc(28rem-4rem)] md:max-w-[calc(28rem-4rem)] snap-center bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex-shrink-0 hover:shadow-md transition-all flex flex-col">
          {/* Card Image Area */}
